...
from botify.recommenders.valrunrecommender import ValRunRecommender
...
recommendations_for_artists_redis = Redis(app, config_prefix="REDIS_RECOMMENDATIONS_FOR_ARTISTS")
recommendations_for_users_redis = Redis(app, config_prefix="REDIS_RECOMMENDATIONS_FOR_USERS")
...
catalog.upload_recommendations_for_users(recommendations_for_users_redis.connection)
catalog.upload_recommendations_for_artists(recommendations_for_artists_redis.connection)
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            ...
            recommender = ValRunRecommender(
                tracks_redis.connection, artists_redis.connection, catalog
                , recommendations_for_artists_redis.connection, recommendations_for_users_redis.connection
                , catalog.top_tracks
            )
        else:
