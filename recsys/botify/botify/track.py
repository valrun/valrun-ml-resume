...
class Catalog:
    ...
    def upload_recommendations_for_artists(self, redis):
        self.app.logger.info(f"Uploading recommendations for artists to redis")
        recommendations_file_path = self.app.config["RECOMMENDATIONS_FOR_ARTISTS"]
        j = 0
        with open(recommendations_file_path) as rf:
            for line in rf:
                recommendations = json.loads(line)
                redis.set(
                    recommendations["artist"], self.to_bytes(recommendations["recommendations"])
                )
                j += 1
        self.app.logger.info(f"Uploaded recommendations for {j} artists")

    def upload_recommendations_for_users(self, redis):
        self.app.logger.info(f"Uploading recommendations for users to redis")
        recommendations_file_path = self.app.config["RECOMMENDATIONS_FOR_USERS"]
        j = 0
        with open(recommendations_file_path) as rf:
            for line in rf:
                recommendations = json.loads(line)
