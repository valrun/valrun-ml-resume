...
class Experiment:
    def __init__(self, name: str, input_file: str, parameters: array, rounds: list):
        self.__name = name
...
class Round:
    def __init__(self, duration, objective: float, n: int):
...
class DriverRound:
    def __init__(self, duration, device, n: int):
        self.__duration = duration
...
class PrefixExperimentLogParser(ExperimentLogParser):
    def __init__(self, prefix: str, parameters: array):
        self.__prefix = prefix
        self.__parameters = parameters

    def parse(self, file_path: str, no_params=False) -> Experiment:
    ...
    @staticmethod
    def filter_lines(lines: array, prefix: str)-> array:
...
class PrefixDriverWithExperimentLogParser(ExperimentLogParser):
    ...
    def parse(self, driver_log_path, file_path: str, no_params=False):
        with open(driver_log_path, 'r') as f_d:
            ...
            with open(file_path, 'r') as f_f:
                ...
                for line in lines_f[0:]:
                    round_prop = json.loads(line)
                    rounds_f.append(
                        Round(rnd_duration[round_prop['duration']], round_prop['objective'], round_prop['round']))

            return Experiment(name, input_file, parameters, rounds_f)

    @staticmethod
    def filter_lines(lines: array, prefix: str):
        return [line[line.find(prefix) + len(prefix):] for line in lines if prefix in line]
...
class ExperimentLogPlotter:
    ...
    def plot(self, filename=None):
        timestamps, objectives = ExperimentLogPlotter._prepare_experiments(self.__experiments, self.__objective_min,
                                                                           self.__ylim)
...
class ExperimentDoubleLogPlotter:
    ...
    def add_min_objectives(self, objective, subplot: int):
        self.__objective_mins[subplot] = objective

    def plot(self, filename=None):
        timestamps, objectives = ExperimentDoubleLogPlotter._prepare_experiments(self.__experiments,
                                                                                 self.__objective_mins, self.__subplots,
                                                                                 self.__ylim)
